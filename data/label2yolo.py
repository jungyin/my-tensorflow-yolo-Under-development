...
if __name__ == '__main__':
    ...
    datas = wider2face(root_path, phase=type)
    for idx, data in enumerate(datas.keys()):
        ...
        labels = datas[data]
        path = data.split('/')
        name,type = path[-1].replace('.jpg','.txt'),path[-2]
        
        save_label_path = f'{save_path}/{type}'
        if not os.path.isdir(save_label_path):
            os.mkdir(save_label_path)
        
        out_txt = f'{save_label_path}/{name}'
